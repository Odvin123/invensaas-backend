...
app.get("/api/test-timezone", verifyToken, async (req, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      server_time_nicaragua: now.toLocaleString("es-NI", {
...
app.get("/api/check-tenant/:tenantId", async (req, res) => {
  ...
  if (!tenantId) {
    return res
      .status(400)
      .json({ exists: false, message: "Tenant ID es obligatorio." });
  }
  try {
    ...
    if (result.rowCount > 0) {
      return res.json({
        exists: true,
        message: "El Tenant ID ya está en uso.",
      });
    } else {
    ...
    res
      .status(500)
      .json({ exists: false, message: "Error interno del servidor." });
  }
});
  ...
  try {
    ...
    if (userResult.rowCount === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Credenciales de acceso no encontradas.",
        });
    }
...
app.post("/api/finalizar-reset-pw", async (req, res) => {
  ...
  try {
    ...
    if (resetResult.rowCount === 0) {
      return res
        ...
        .json({
          success: false,
          message:
            "Token de restablecimiento inválido o expirado. Vuelva a empezar.",
        });
    }
    ...
    console.error(
      "Error al finalizar el restablecimiento de contraseña:",
      error,
    );
    res
      .status(500)
  ...
  try {
    ...
    if (userResult.rowCount === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Solicitud de cambio inválida o ya procesada.",
        });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Contraseña actualizada correctamente. Inicie sesión.",
      });
  } catch (error) {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Credenciales inválidas o Tenant ID incorrecto.",
        });
    }
...
app.post("/api/invitaciones/generar", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const baseUrl =
      process.env.FRONTEND_URL || "https://invensaas-sistema.vercel.app";
...
app.delete("/api/empresa/:tenantId", verifyToken, async (req, res) => {
  ...
  if (tenantId === "super_admin") {
    return res.status(403).json({
      success: false,
      message:
        "Error: El puesto de Administración Central (super_admin) no puede ser eliminado.",
    });
  }
  ...
  try {
    ...
    // ============================================
    await client.query("DELETE FROM cortes_caja WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // 5. ELIMINAR VENTAS
    // ============================================
    await client.query("DELETE FROM ventas WHERE empresa_id = $1", [empresaId]);
    console.log("✅ Ventas eliminadas");

    // ============================================
    // 6. ELIMINAR CONTROL DE FOLIOS
    // ============================================
    await client.query("DELETE FROM control_folios WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // ============================================
    await client.query("DELETE FROM proveedores WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // ============================================
    await client.query("DELETE FROM categorias WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // ============================================
    await client.query("DELETE FROM clientes WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // ============================================
    await client.query("DELETE FROM vendedores WHERE empresa_id = $1", [
      empresaId,
    ]);
    ...
    // ============================================
    await client.query("DELETE FROM usuarios WHERE empresa_id = $1", [
      empresaId,
    ]);
...
app.post("/api/register", async (req, res) => {
  ...
  const emailRegex =
    /^[^\s@]+@(gmail\.com|outlook\.com|yahoo\.com|icloud\.com)$/i;

  if (!emailRegex.test(correo_electronico)) {
    return res.status(400).json({
      success: false,
      message:
        "El formato de correo es inválido o el dominio no está permitido.",
    });
  }
  ...
  try {
    ...
    if (tokenResult.rowCount === 0) {
      return res.status(400).json({
        success: false,
        message:
          "El enlace de invitación no es válido, ya fue usado o ha expirado.",
      });
    }
...
app.get("/api/check-email/:email", async (req, res) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ exists: false, message: "Correo es obligatorio." });
  }
...
app.get("/api/admin/empresas", verifyToken, async (req, res) => {
  if (req.usuario.rol !== "super_admin") {
    return res
      .status(403)
      .json({ success: false, message: "Acción no permitida para este rol." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error interno al cargar datos." });
  }
});
  ...
  try {
    ...
    if (userResult.rowCount === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Administrador no encontrado para este Puesto/Empresa.",
        });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Error interno al procesar el reseteo y/o enviar el correo.",
      });
  }
});
