...
router.post(
  "/",
  verifyToken,
  checkRole(["administrador", "super_admin"]),
  async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Datos incompletos o inválidos (Empresa, Detalles o Pagos faltantes).",
        });
    }
    ...
    try {
      ...
      if (clienteFinal) {
        ...
        if (clienteResult.rowCount === 0) {
          ...
          if (defaultCliente.rowCount > 0) {
            ...
            console.log(
              `⚠️ Cliente ${cliente_id} no encontrado, usando "público general" (ID: ${clienteFinal})`,
            );
          } else {
            ...
            console.log(
              `✅ Cliente "público general" creado con ID: ${clienteFinal}`,
            );
          }
        }
      } else {
      ...
      console.log(
        `📌 Cliente ID: ${clienteFinal}, Vendedor ID: ${vendedorFinal}`,
      );
      ...
      if (folioResult.rows.length === 0) {
        await client.query(
          "INSERT INTO control_folios (empresa_id, ultimo_folio) VALUES ($1, 1)",
          [empresa_id],
        );
        nuevoFolio = 1;
      } else {
        ...
        await client.query(
          "UPDATE control_folios SET ultimo_folio = $1 WHERE empresa_id = $2",
          [nuevoFolio, empresa_id],
        );
      }
      ...
      for (const detalle of detalles) {
        ...
        const descuentoProducto = parseFloat(detalle.descuento) || 0; // ✅ Obtener descuento del producto
        ...
        if (productoResult.rows.length === 0) {
          throw new Error(
            `Producto ID ${producto_id} no encontrado en la empresa.`,
          );
        }
        ...
        if (stockActual < cantidad) {
          throw new Error(
            `Stock insuficiente para Producto ID ${producto_id}. Stock: ${stockActual}, solicitado: ${cantidad}.`,
          );
        }
        ...
        if (descuentoProducto > subtotalDetalle) {
          throw new Error(
            `El descuento (C$${descuentoProducto.toFixed(2)}) no puede ser mayor al subtotal del producto (C$${subtotalDetalle.toFixed(2)}).`,
          );
        }
      ...
      if (totalPagado < totalVentaCalculado) {
        throw new Error(
          `El total pagado (${totalPagado.toFixed(2)}) es menor que el total de la venta (${totalVentaCalculado.toFixed(2)}).`,
        );
      }
      ...
      for (const detalle of detallesCompletos) {
        ...
        const stockAntes = parseFloat(stockResult.rows[0].stock) || 0;
        const stockDespues = stockAntes - detalle.cantidad;

        await client.query(
          `INSERT INTO movimientos_inventario 
                 (empresa_id, producto_id, tipo, cantidad, stock_antes, stock_despues, usuario_id, referencia, motivo)
...
router.get("/folio_actual", verifyToken, async (req, res) => {
  ...
  if (!empresa_id) {
    return res
      .status(401)
      .json({ success: false, message: "ID de empresa no encontrado." });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Error interno del servidor al obtener el folio.",
      });
  } finally {
...
router.get("/productos-vendidos", verifyToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al generar el reporte." });
  }
});
